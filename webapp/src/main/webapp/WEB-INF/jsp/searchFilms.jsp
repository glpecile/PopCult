        ...
        <div class="dropdown pr-4">
            ...
            <ul class="dropdown-menu shadow-lg" aria-labelledby="dropdownMenuButton1">
                <c:forEach var="type" items="${sortTypes}">
                    <li><a class="dropdown-item" href="<c:url value=""><c:param name="sort" value="${type}" /><c:param name="genre" value="${param.genre}" /><c:param name="term" value="${param.term}"/></c:url>"><c:out value="${type}"/></a></li>
                </c:forEach>
            </ul>
        </div>
        <div class="dropdown pr-4">
            ...
            <ul class="dropdown-menu shadow-lg" aria-labelledby="dropdownMenuButton2">
                <c:forEach var="type" items="${genreTypes}">
                    <li><a class="dropdown-item" href="<c:url value=""><c:param name="sort" value="${param.sort}" /><c:param name="genre" value="${type}" /><c:param name="term" value="${param.term}"/></c:url>"><c:out value="${type}"/></a></li>
                </c:forEach>
            </ul>
        </div>
    </div>

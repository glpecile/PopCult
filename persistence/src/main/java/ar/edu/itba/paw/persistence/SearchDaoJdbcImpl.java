...
//
//    @Override
//    public PageContainer<MediaList> searchListMediaByName(String name, int page, int pageSize, SortType sort) {
...
//        List<Media> elements = jdbcTemplate.query("SELECT * FROM media WHERE title ILIKE CONCAT('%', ?, '%') AND type = ?  AND mediaid NOT IN (SELECT mediaid FROM listelement WHERE medialistid = ?)" + orderBy + " OFFSET ? LIMIT ?", new Object[]{title, mediaType, mediaList, page * pageSize, pageSize}, MEDIA_ROW_MAPPER);
//        int totalCount = jdbcTemplate.query("SELECT COUNT(*) FROM media WHERE title ILIKE CONCAT('%', ?, '%') AND type = ? AND mediaid NOT IN (SELECT mediaid FROM listelement WHERE medialistid = ?)", new Object[]{title, mediaType, mediaList}, COUNT_ROW_MAPPER).stream().findFirst().orElse(0);
//        return new PageContainer<>(elements, page, pageSize, totalCount);
//    }
//
//    @Override
